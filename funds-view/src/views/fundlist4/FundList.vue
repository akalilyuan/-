  ...
  <div class="funds-list">
    ...
    <div class="table_container">
      <el-table :data="tableData" style="width: 100%" max-height="450" stripe>
        ...
        <el-table-column label="公告标题" width="120" prop="type">
          <template #default="scope">
            <span style="color: #f56767">{{ scope.row.type }}</span>
          </template>
        </el-table-column>
        <!-- <el-table-column label="公告内容" width="130" prop="description">
        </el-table-column> -->
        <el-table-column label="公告内容" width="100" prop="income">
          <template #default="scope">
            <span >{{ scope.row.income }}</span>
          </template>
        </el-table-column>
        <el-table-column label="发布时间" width="100" prop="expenditure">
          <template #default="scope">
...
import DialogVue from "@/components/Dialog4.vue";
...
const handleDelete = (index, row) => {
  proxy.$axios.post(`/funds4/delete4/${row.id}`).then((res) => {
    proxy.$message("删除成功！");
    getProfile();
  });
};
