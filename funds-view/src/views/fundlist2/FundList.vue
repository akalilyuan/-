  ...
  <div class="funds-list">
    ...
    <div class="table_container">
      <el-table :data="tableData" style="width: 100%" max-height="450" stripe>
        ...
        <el-table-column label="栋区" width="120" prop="type">
        </el-table-column>
        <el-table-column label="楼层" width="100" prop="income">
          <template #default="scope">
            <span >{{ scope.row.income }}</span>
          </template>
        </el-table-column>
        <el-table-column label="门牌号" width="150" prop="remark">
        </el-table-column>
        <el-table-column label="租用原因" width="100" prop="expenditure">
          <template #default="scope">
            <span >{{ scope.row.expenditure }}</span>
          </template>
        </el-table-column>
        <el-table-column label="租用时间" width="150" prop="account">
          <template #default="scope">
            <span>{{ scope.row.account }}</span>
          </template>
        </el-table-column>
        <el-table-column label="租用描述" width="130" prop="description">
        </el-table-column>
...
import DialogVue from "@/components/Dialog2.vue";
...
const handleDelete = (index, row) => {
  proxy.$axios.post(`/funds2/delete2/${row.id}`).then((res) => {
    proxy.$message("删除成功！");
    getProfile();
  });
};
