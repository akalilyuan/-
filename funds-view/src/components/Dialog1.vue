      ...
      <div class="form">
        ...
        >
          ...
          <el-form-item prop="remark" label="门牌号:" label-width="140px">
            <el-input type="description" v-model="formData.remark"></el-input>
          </el-form-item>

          <el-form-item prop="expenditure" label="维修内容:" label-width="140px">
            <el-input
              type="expenditure"
          ...
          <el-form-item prop="account" label="维修状况:" label-width="140px">
            <el-select v-model="formData.account" placeholder="维修状况">
              <el-option
                v-for="(formtype1, index1) in form_type_list1"
                :key="index1"
                :label="formtype1"
                :value="formtype1"
              ></el-option></el-select>
...
const form_rules = reactive({
  ...
  expenditure: [{ required: true, message: "维修内容不能为空!", trigger: "blur" }],
  account: [{ required: true, message: "维修状况不能为空!", trigger: "blur" }],
  type: [{ required: true, message: "栋区不能为空!", trigger: "blur" }],
  remark: [{ required: true, message: "门牌号不能为空!", trigger: "blur" }],
});
...
const onSubmit = (form) => {
  form.validate((valid) => {
    if (valid) {
      const url =
        props.dialog.option == "add" ? "add1" : `edit1/${props.formData.id}`;
      proxy.$axios.post(`/funds1/${url}`, props.formData).then((res) => {
        // 编辑成功
