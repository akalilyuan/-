    ...
    <el-dialog :title="dialog.title" v-model="dialog.show">
      <div class="form">
        ...
        >
          ...
          <el-form-item prop="expenditure" label="缴费时间:" label-width="140px">
            <el-input
              type="expenditure"
          ...
          <el-form-item prop="account" label="物业费:" label-width="140px">
...
const form_rules = reactive({
  ...
  expenditure: [{ required: true, message: "缴费时间不能为空!", trigger: "blur" }],
  account: [{ required: true, message: "物业费不能为空!", trigger: "blur" }],
...
const onSubmit = (form) => {
  form.validate((valid) => {
    if (valid) {
      const url =
        props.dialog.option == "add" ? "add3" : `edit3/${props.formData.id}`;
      proxy.$axios.post(`/funds3/${url}`, props.formData).then((res) => {
        // 编辑成功
